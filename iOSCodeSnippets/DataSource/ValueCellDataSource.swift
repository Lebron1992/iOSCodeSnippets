...
class ValueCellDataSource: NSObject {
    ...
    final func deleteRows<Cell: ValueCell, Value: Any>(
        at indexPaths: [IndexPath],
        cellClass: Cell.Type
    ) where Cell.Value == Value {
        indexPaths.forEach {
            guard let item = self.item(at: $0, cellClass: cellClass) else { return }
            deleteRow(
                value: item,
                cellClass: Cell.self,
                atIndex: $0.row,
    ...
    func item<Cell: ValueCell, Value: Any>(
        at indexPath: IndexPath,
        cellClass: Cell.Type
    ) -> Value? where Cell.Value == Value {
        let value = self[indexPath] as? Value
        return value
    }
